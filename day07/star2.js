...
const canBeMadeTrue = eq => {
    ...
    while (operands.length > 1) {
        ...
        const concats = possibleIntermediates
            .filter(n => ('' + n).endsWith(operand))
            .map(n => {
                const s = '' + n;
                const suf = '' + operand;
        ...
        if (test)
            console.log(
                `${possibleIntermediates} / ${operand} => ${divs}   |||` +
                `   ${possibleIntermediates} - ${operand} => ${subs}   |||` +
                `   ${possibleIntermediates} || ${operand} => ${concats}`);

        possibleIntermediates = [...divs, ...subs, ...concats];
    }
